...
async function statsCommand(options) {
  ...
  try {
    ...
    // Analyze existing proof generation data if available
    await analyzeExistingProofData();       
...
async function analyzeExistingProofData() {
  ...
  try {
    ...
    // Look for existing proof files and logs
    const proofDir = path.join(projectRoot, 'proof');
    const outputsDir = path.join(projectRoot, 'outputs');
    
    if (!fs.existsSync(proofDir) && !fs.existsSync(outputsDir)) {
      console.log(chalk.yellow('No proof data found. Run "zisk-dev prove" first to generate proofs.'));
      return;
    }
    ...
    for (const logFile of logFiles) {
      if (fs.existsSync(logFile)) {
        ...
        // Look for Air instances information in the logs
    ...
    if (!foundLogs) {
      console.log(chalk.yellow('\nNo detailed proof generation logs found.'));
      console.log(chalk.yellow('The logs may not contain Air instances data.'));
      console.log(chalk.cyan('\nTo get detailed proof generation analysis:'));
      console.log(chalk.cyan('1. Run "zisk-dev prove" to generate proofs'));
      console.log(chalk.cyan('2. The detailed Air instances and memory usage will be shown during proof generation'));
      console.log(chalk.cyan('3. Run "zisk-dev stats" again to see the analysis'));
    }
    
  } catch (error) {
    console.log(chalk.red(`\nFailed to analyze existing proof data: ${error.message}`));
  }
}
